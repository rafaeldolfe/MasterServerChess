	...
	public abstract class CustomEditorBaseDrawer<TSelf, TTarget> : UnityObjectDrawer<TSelf, TTarget>, ICustomEditorDrawer where TSelf : CustomEditorBaseDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		protected float GetOptimalPrefixLabelWidthForEditor(Editor editor, int indentLevel)
		{
			if(editor == null)
			{
				...
				#endif
				return MinAutoSizedPrefixLabelWidth;
			}
			...
			{
				#if DEV_MODE
				Debug.LogWarning("GetOptimalPrefixLabelWidthForEditor Editor serializedObject had no properties."); // this can happen when target has no serialized fields at all?
				#endif
				return MinAutoSizedPrefixLabelWidth;
			}

			var tempLabel = GUIContentPool.Empty();
			float widestPrefixWidth = MinAutoSizedPrefixLabelWidth;
			do
			{
			...
			return Mathf.Clamp(widestPrefixWidth, MinPrefixLabelWidth, MaxPrefixLabelWidth);
		}
		...
		private void UpdateHeightEstimate()
		{
			if(!UsesEditorForDrawingBody)
			{
				...
				{
					unfoldedHeight = UnityObjectDrawerUtility.CalculateHeight(this);
				}
				else
				{
					unfoldedHeight = HeaderHeight;
				}
			}
			...
			{
				unfoldedHeight = HeaderHeight + PrefixResizerDragHandleHeight + ControlsTopMargin + element.resolvedStyle.height + ControlsBottomMargin;
				usingInitialEstimatedHeight = false;
			}
			#endif
			else
			{
				unfoldedHeight = EstimatedUnfoldedHeight;
		...
		public override bool Draw(Rect position)
		{
			...
			if(!HeadlessMode && DrawPrefix(pos))
			{
				dirty = true;
			}
			pos.y += pos.height;
			float dragHandleHeight = PrefixResizerDragHandleHeight;
			pos.y += dragHandleHeight;
			...
			if(MembersAreVisible)
			{
				...
				pos.height = usingInitialEstimatedHeight ? EstimatedUnfoldedHeight : unfoldedHeight - headerHeight - dragHandleHeight;
		...
		protected virtual bool DrawMembers(Rect position)
		{
			...
			// UPDATE: Testing new method where height is instantly updated even outside of the repaint event
			// if height is using its default value, in order to make the GUI get its layout right as fast as possible
			// if(eventType == EventType.Repaint || height == InitialTempHeight)
			// UPDATE: testing without this
			if(eventType == EventType.Repaint)
			{
				var rectEnd = GUILayoutUtility.GetLastRect();
				float yEnd = rectEnd.y - rectEnd.height;
				float bodyHeight = yEnd - position.y;
				float setHeight = HeaderHeight + PrefixResizerDragHandleHeight + bodyHeight;
