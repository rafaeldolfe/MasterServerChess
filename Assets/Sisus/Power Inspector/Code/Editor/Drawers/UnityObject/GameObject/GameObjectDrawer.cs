...
namespace Sisus
{
	...
	public class GameObjectDrawer : ParentDrawer<Component[]>, IGameObjectDrawer, IOnProjectOrHierarchyChanged, IEnumerable<IComponentDrawer>
	{
		...
		/// </summary>
		private static readonly object[] CopyComponentToGameObjectParams = {null, null, false, new List<Component>(1)}; //CopyComponentToGameObject(Component component, GameObject targetGameObject, bool validateOnly, [NotNull] List<Component> outNewComponents);
		...
		protected float HeaderHeightOutsideDebugMode
		{
			get
			{
				if(isPrefab)
				{
					return EditorGUIDrawer.GameObjectTitlebarHeight(AddressablesUtility.IsInstalled) + GameObjectHeaderDrawer.OpenInPrefabModeButtonHeight;
				}
				return EditorGUIDrawer.GameObjectTitlebarHeight(isPrefabInstance);
			}
		}
		...
		private void OptimizeMemberPrefixLabelWidthsInUnison()
		{
			...
			float prefixWidth = Mathf.Clamp(GetOptimalPrefixLabelWidth(0), DrawGUI.MinAutoSizedPrefixLabelWidth, DrawGUI.MaxAutoSizedPrefixLabelWidth);

			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(prefixWidth > 0, ToString()+ ".OptimizeMemberPrefixLabelWidthsInUnison result " + prefixWidth+" less than zero");
			Debug.Assert(prefixWidth >= DrawGUI.MinAutoSizedPrefixLabelWidth, ToString()+ ".OptimizeMemberPrefixLabelWidthsInUnison result " + prefixWidth+" less than MinAutoSizedPrefixLabelWidth "+DrawGUI.MinAutoSizedPrefixLabelWidth);
			#endif

			// Skip transform and add component menu
			for(int n = members.Length - LastCollectionMemberCountOffset; n >= 1; n--)
			{
				var comp = members[n] as IComponentDrawer;
				if(comp != null)
				{
					comp.PrefixLabelWidth = Mathf.Clamp(prefixWidth, comp.MinPrefixLabelWidth, comp.MaxPrefixLabelWidth);
				}
			}
		}
