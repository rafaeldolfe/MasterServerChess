...
namespace Sisus
{
	...
	{
		private static Stack<List<Component[]>> componentsListPool = new Stack<List<Component[]>>();
		protected Dictionary<string, List<Component[]>> categorizedBuildList = new Dictionary<string, List<Component[]>>();
		...
		protected void ClearCategorizedBuildList()
		{
			foreach(var componentsList in categorizedBuildList.Values)
			{
				...
				{
					var array = componentsList[n];
					ArrayPool<Component>.Dispose(ref array);
				}
