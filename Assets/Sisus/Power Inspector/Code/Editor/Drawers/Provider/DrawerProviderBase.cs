...
namespace Sisus
{
	...
	public abstract class DrawerProviderBase : IDrawerProvider
	{
		...
		public IFieldDrawer GetForField([CanBeNull]object value, [NotNull]Type fieldType, [CanBeNull]LinkedMemberInfo memberInfo, [CanBeNull]IParentDrawer parent, [CanBeNull]GUIContent label, bool readOnly, bool ignoreAttributes = false)
		{
			...
			drawerType = GetDrawerTypeForField(fieldType, ignoreAttributes);
		...
		protected virtual Type GetDrawerTypeForAssets([NotNull]Object[] assets, [CanBeNull]out Object[] assetImporters, [CanBeNull]out Type customEditorType)
		{
			...
			return GetDrawerTypeForAsset(assets[0], false);
		}
		#else
		[NotNull]
		...
		[NotNull]
		protected virtual Type GetDrawerTypeForAsset([NotNull]Object asset, bool ignoreAttributes = false)
		{
			#if UNITY_EDITOR
			#if DEV_MODE && DEBUG_GET_FOR_ASSET
			...
			return GetDrawerTypeForAsset(asset.GetType(), localPath, ignoreAttributes);
			#else
		...
		[NotNull]
		protected virtual Type GetDrawerTypeForAsset([NotNull]Type assetType, [NotNull]string localPath, bool ignoreAttributes = false)
		{
			Type drawerType;
